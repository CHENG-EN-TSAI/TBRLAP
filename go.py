...
def load_and_resize(data_path, img_size=(256, 256)):
    ...
    pil_f = PImage.open(fixed_path).convert("L").resize(img_size, resample=PImage.BILINEAR)
    pil_m = PImage.open(moving_path).convert("L").resize(img_size, resample=PImage.BILINEAR)

    # PIL size is (W, H)
    Wf0, Hf0 = np.asarray(data_path["fixed_size"], dtype=np.float64)
    Wm0, Hm0 = np.asarray(data_path["moving_size"], dtype=np.float64)
...
def main():
    ...
    for i in range(100):
        ...
        initial_field = ants_affine_registration(im_f, im_m, seed=args.seed)
        ...
        metrics = {"fixed_image": data_paths[i].get("fixed", ""),
                   "moving_image": data_paths[i].get("moving", "")}
        ...
        if backward:
            ...
            initial_field = ants_affine_registration(im_m, im_f, seed=args.seed)
